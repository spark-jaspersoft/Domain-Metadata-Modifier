...
"""
    ...
    def testRemoveFieldFromSchemaIsFK(self):
        ...
        try:
            self.domain_action.renameOrRemoveFieldFromSchema(root=mock_root, fieldname='name2', newfieldname=None, newdbcolname=None, log=self.log)
    ...
    def testRenameFieldInSchemaIsItem(self):
        ...
        node_list = [mock_node1, mock_node2]
        mock_root.xpath = MagicMock(side_effect = [[], node_list, []])
        self.domain_action.renameOrRemoveFieldFromSchema(root=mock_root, fieldname='name2', newfieldname='name4', newdbcolname=None, log=self.log)
        mock_node2.attrib.__setitem__.assert_called_with(Common.RESOURCE_ID, 'name4')
    ...
    def testRenameFieldInSchemaIsField(self):
        ...
        mock_root.xpath = MagicMock(side_effect = [[], [], node_list])
        self.domain_action.renameOrRemoveFieldFromSchema(root=mock_root, fieldname='name2', newfieldname='name4', newdbcolname=None, log=self.log)
        mock_node2.attrib.__setitem__.assert_called_once_with(Common.ID, 'name4')
    ...
    def testRenameFieldInSchemaIsFieldWithDBCol(self):
        ...
        node_list = [mock_node1, mock_node2]
        mock_root.xpath = MagicMock(side_effect = [[], [], node_list])
        self.domain_action.renameOrRemoveFieldFromSchema(root=mock_root, fieldname='name2', newfieldname='name4', newdbcolname='name4', log=self.log)
        mock_node2.attrib.__setitem__.assert_called_with(Common.FIELD_DB_NAME, 'name4')
        
    def testRemoveField(self):
        field_name = 'name1'
        self.domain_action.removeOrRenameField(filename=self.schema_filepath, fieldname=field_name, newfieldname=None, newdbcolname=None, log=self.log)
    ...
    def testRenameDBField(self):
        oldfieldname = 'name1'
        newfieldname = 'name4'
        self.domain_action.removeOrRenameField(filename=self.schema_filepath, fieldname=oldfieldname, newfieldname=newfieldname, newdbcolname=newfieldname, log=self.log)
        with open(self.schema_filepath) as h:
            schema_xml = h.read()
            self.assertEqual(self.DOMAIN_SCHEMA_RENAME_NAME1_TO_NAME4_RENAME_DB.replace(' ', ''), schema_xml.replace(' ', ''), 'Domain schema file does not match expected')
    ...
    def testRenameMultipleFields(self):
        ...
        self.domain_action.removeOrRenameField(filename=self.schema_filepath, fieldname=oldfieldname, newfieldname=newfieldname, newdbcolname=None, log=self.log)
        with open(self.schema_filepath) as h:
            schema_xml = h.read()
    ...
    def testRenameMultipleFieldsWithDB(self):
        ...
        self.domain_action.removeOrRenameField(filename=self.schema_filepath, fieldname=oldfieldname, newfieldname=newfieldname, newdbcolname=newdbcolname, log=self.log)
        with open(self.schema_filepath) as h:
            schema_xml = h.read()
            self.assertEqual(self.DOMAIN_SCHEMA_RENAME_NAME1_TO_NAME4_AND_NAME2_TO_NAME5_RENAME_DB.replace(' ', ''), schema_xml.replace(' ', ''), 'Domain schema file does not match expected')
